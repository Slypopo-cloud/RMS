...
import { OrderFilters } from "@/components/global/order-filters";
import { 
    User as UserIcon, 
    Package, 
    CheckCircle2, 
    ...
    const filteredOrders = useMemo(() => {
        return orders.filter(order => {
            ...
            const matchesSearch = !searchQuery || 
                ...
                (order.restaurantTable?.number && order.restaurantTable.number.includes(searchQuery));
            ...
            <div className="glass-card rounded-3xl overflow-hidden shadow-2xl border-slate-800">
                <div className="p-8 border-b border-slate-800 bg-slate-900/40 flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                        {filteredOrders.length} / {orders.length} ENTRIES
                    </span>
                </div>
                                    ...
                                    <td colSpan={8} className="px-8 py-20 text-center">
                                        <div className="flex flex-col items-center justify-center text-slate-600 gap-4">
                                            ...
                                            <p className="font-bold uppercase tracking-widest text-xs italic">
                                                {orders.length === 0 ? "Historical database is currently empty." : "No orders match your filters."}
                                            </p>
                                        </div>
                                    </td>
                                </tr>
